...
gen_js_instance = function(data,
    ...
    dfile = gen_js_datafiles(data, outdir, js.type, name.col = name.col, ref = ref, dataset_name = dataset_name, tree = tree, figure = figure, coordinates = coordinates)

    if (js.type == 'PGV'){
        # generate the samples.json file
        gen_samples_json(data, outdir, dataset_name, metadata.fields, name.col)
    }
...
#' 
#' @export
gen_js_datafiles = function(data, outdir, js.type, name.col = NA, meta_col = NA, ref = NA, dataset_name = NA, tree = NA, figure = NA, coordinates = NA){
    ...
    if (js.type == 'PGV'){
        ...
        coordinates = read_coordinates(coordinates, outdir = outdir, dataset_name = dataset_name, data = data, name.col = name.col)
        if (!is.na(coordinates)){
            # set up the figure (AKA anatomy or body map)
            figure = read_figure(figure, outdir = outdir, dataset_name = dataset_name)
            figure_plot = list("sample" = NA_character_,
                             "type" = "anatomy",
                             "source" = coordinates,
                             "figure" = figure,
                             "title" = dataset_name,
                             "visible" = TRUE)
            item$plots = c(list(figure_plot), item$plots)
        } else {
            if (!is.na(figure)){
                warning('Looks like a figure file was provided but no coordinates and hence the figure will not be included. Skipping.')
            } else {
                warning('No coordinates file was provided so the anatomy figure will not be visible.')
            }
        }
...
#' @name gen_samples_json
#' @description internal
#'
#' Generate the JSON file containing the samples metadata
#'
#' @param outdir the path to the PGV/gGnome.js repository clone
#' @param js.type either "PGV" or "gGnome.js"
gen_samples_json = function(data, outdir, dataset_name, metadata.fields, name.col){
        samples.fn = paste0(outdir, '/public/data/', dataset_name, '/samples.json')
        if (!is.na(metadata.fields)){
            metadata.fields = intersect(metadata.fields, names(data))
            if (length(metadata.fields) == 0){
                metadata.fields = NA
            } else {
                samples.list = list()
                for (nm in data[, unique(get(name.col))]){
                    # iterate on sample names here
                    samples.list[[nm]] = as.list(data[get(name.col) == nm,
                                                       ..metadata.fields])
                }
            }
        }
        if (is.na(metadata.fields)){
            message('No metadata supplied for samples. Generating empty samples.json')
            samples.list = list()
            for (nm in data[, unique(get(name.col))]){
                # adding "sample" as a field so that the sample metadata is not completely empty
                # everything works fine even if we don't do this, but the hover-over menu is kind of ugly if there is no info
                samples.list[[nm]] = list(sample = nm)
            }
        }

        message('Writing samples metadata to: ', samples.fn)
        jsonlite::write_json(samples.list, samples.fn,
                             pretty=TRUE, auto_unbox=TRUE, digits=4)
}
...
#' @description
#'
#' Convert PNG to an SVG in the format expected by PGV
#'
#' The conversion includes the following steps:
#'   1. Get the dimensions of the PNG using pgv::readPNG
#'   2. Rescale the dimensions to match the size expected by PGV (height = 885 pixles)
#'   3. Convert the PNG to a string of Base64
#'   4. Use template.svg and update the height, width and the Base64 string
#'   5. Save the output svg. That's it.
#'
#' @param figure path to PNG
#' @param fn path to output svg
#' @export
png2pgv = function(figure, fn){
    # read the dimensions of the PNG
    p = tryCatch(png::readPNG(figure),
                 error = function(e){
                     stop('Input figure must be a PNG, but the figure you provided ("', figure, '") does not appear to be in PNG format')
                 })
    d = dim(p)
    h = d[1]
    w = d[2]
    # resize figure to match the size expected by PGV
    hm = 885
    wm = w*hm/h

    # convert png to 64 bit
    p64 = base64enc::base64encode(figure)

    # load the template SVG and update width, height, and the 64-bit image
    template.svg = readLines(system.file('extdata', 'template.svg', package="gGnome"))
    svg.out = lapply(template.svg, function(l){
        l = gsub('HEIGHT', as.character(round(hm, 2)), l)
        l = gsub('WIDTH', as.character(round(wm, 2)), l)
        l = gsub('XXX', p64, l)
        return(l)
    })
    writeLines(unlist(svg.out), fn)
}
...
read_figure = function(figure, outdir, dataset_name){
    fn = 'figure.svg'
    svg_path = paste0(outdir, '/public/data/', dataset_name, '/', fn)
    if (is.na(figure)){
        message('No figure was provided hence using the default figure (human body sketch)')
        default = system('extdata', 'figure.svg', package="gGnome")
        system(paste('cp', default, svg_path))
    } else {
        if (grepl('.svg$', figure)){
            message('Copying input svg "', figure, '" to: "', svg_path, '"')
            system(paste('cp', figure, svg_path))
        } else {
            if (grepl('.png', figure)){
                message('Converting input png to svg.')
                png2pgv(figure, svg_path)
            } else {
                warning('Invalid input figure provided. Only ".png" and ".svg" files are acceptable. Will use the default figure instead. Notice that things might not make sense.')
                default = system('extdata', 'figure.svg', package="gGnome")
                system(paste('cp', default, svg_path))
            }
        }
...
#' @description internal
#'
#' Read the coordinates file and make sure it is properly formatted
#'
#' @param coordinates path to coordinates json file
#' @param outdir the path to the PGV/gGnome.js repository clone
#' @param dataset_name the name of the dataset. Only relevant for PGV. This should be the name of the project that all the samples belong to. You must provide a name since PGV stores all the data under a folder matching your dataset name. This allows a single PGV instance to include multiple datasets which could be browsed by going to the "Data Selection" page in the browser
#' @return path to the svg
read_coordinates = function(coordinates, outdir, dataset_name, data, name.col){
    fn = 'anatomy.json'
    fn_path = paste0(outdir, '/public/data/', dataset_name, '/', fn)
    if (!is.na(coordinates)){
        # FIXME:
        # for now just copying the coordinates file into its destination and assuming things are good
        system(paste('cp', coordinates, fn_path))
        return(fn)
    } else {
        if ('x' %in% names(data) & 'y' %in% names(data)){
            message('Coordinates supplied in data table.')
            message('Generating ', fn_path)
            jsonlite::write_json(data[,.(sample = get(name.col), x, y)], fn_path,
                             pretty=TRUE, auto_unbox=TRUE, digits=4)
            return(fn)
        }
    }
