...
#' @param pad Padding to add around each breakend
#' @param find_duplicates Only search for duplicated SVs in a single set of SVs (jun_a and not jun_b must be provided)
#' @param ignore.strand Ignore breakend strands
#' @author Kevin Hadi
#' @export fra.overlaps
fra.overlaps = function(jun_a, jun_b, pad = 0, arr.ind = TRUE, ignore.strand = FALSE, return.as.data.table = FALSE, find_duplicates = FALSE, sort_junctions = FALSE) {
  is_jun_b_arg_valid_opt1 = find_duplicates && missing(jun_b)
  is_jun_b_arg_valid_opt2 = find_duplicates && !missing(jun_b) && is.null(jun_b)
  is_jun_b_arg_valid = find_duplicates && (is_jun_b_arg_valid_opt1 || is_jun_b_arg_valid_opt2)
  invalid_arguments = find_duplicates && !is_jun_b_arg_valid
  ...
  if (find_duplicates) {
    ...
    junpiv_b = gUtils::grl.pivot(jun_b)
  }
  ra_a = junpiv_a
  ra_b = junpiv_b
  ...
  convert_to_dt = function(x) data.table::setDT(GenomicRanges::as.data.frame(x))

  fov_a1b1 = convert_to_dt(GenomicRanges::findOverlaps(bpa_1, bpb_1, ignore.strand = ignore.strand))
  fov_a2b2 = convert_to_dt(GenomicRanges::findOverlaps(bpa_2, bpb_2, ignore.strand = ignore.strand))

  if (!sort_junctions) {
    fov_a1b2 = convert_to_dt(GenomicRanges::findOverlaps(bpa_1, bpb_2, ignore.strand = ignore.strand))
    fov_a2b1 = convert_to_dt(GenomicRanges::findOverlaps(bpa_2, bpb_1, ignore.strand = ignore.strand))
  }

  if (find_duplicates) {
    fov_a1b1 = fov_a1b1[queryHits != subjectHits]
    fov_a2b2 = fov_a2b2[queryHits != subjectHits]
  }

  subset_inverse = find_duplicates && !sort_junctions
  if (subset_inverse) {
    fov_a1b2 = fov_a1b2[queryHits != subjectHits]
    fov_a2b1 = fov_a2b1[queryHits != subjectHits]
  }
  

  hits = hitscomb1 = data.table::merge.data.table(fov_a1b1, fov_a2b2, by = c("queryHits", "subjectHits"))
  if (!sort_junctions) {
    hitscomb2 = data.table::merge.data.table(fov_a2b1, fov_a1b2, by = c("queryHits", "subjectHits"))
    hits = data.table:::unique.data.table(data.table:::rbind.data.table(hitscomb1, hitscomb2))
  }

  is_hits_empty = NROW(hits) == 0
  return_empty_matrix = !return.as.data.table && is_hits_empty
  return_filled_matrix = !return.as.data.table && !is_hits_empty
  names(hits) = c("ra1.ix", "ra2.ix")
  if (return_filled_matrix) {
    hits = as.matrix(hits)
  }
  if (return_empty_matrix) {
    hits = as.matrix(cbind(ra1.ix = NA_real_, ra2.ix = NA_real_))
  }
  return(hits)  

}

#' Fast RA duplicate find
#' 
#' Function that supersedes ra.duplicated
#' 
#' See fra.overlaps for details on
#' implementation of fra.overlaps.
#' find_duplicates allows only non-self
#' matches to be considered inside fra.overlaps.
#' @param pad Padding
#' @param ignore.strand Ignore breakend strands
#' @author Kevin Hadi
#' @export fra.duplicated
fra.duplicated = function(grl, pad = 500, ignore.strand = FALSE, sort_junctions = TRUE) {
      if (!is(grl, "GRangesList")){
        stop("Error: Input must be GRangesList!")
    }
    ...
    if (length(grl)>1){
        ...
        ix.pair = fra.overlaps(jun_a = grl, pad=pad, ignore.strand = ignore.strand, return.as.data.table = TRUE, find_duplicates = TRUE, sort_junctions = sort_junctions)[ra1.ix!=ra2.ix]
