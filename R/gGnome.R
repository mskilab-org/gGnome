...
setValidity("gwalks",
            function(object){
                ...
                if (isEmpty(object)){
                    ...
                    message("Required metadata fields: cn, str, and is.cycle.")
                } 
                else {
                    return(TRUE)
                }
...
gwalks = function(...){
    ...
    if (!"cn" %in% coln) {
        if (verbose){
            warning("Arbitrarily setting all walk 'cn' to 1.")
        } 
        values(grl)$cn = 1
    }
    if (!"str" %in% coln) {
        if (verbose) {
            warning("Arbitrarily setting all walk 'str' to '+'.")
        }
        values(grl)$str = "+"
    }
    if (!"is.cycle" %in% coln) {
        if (verbose) {
            warning("Arbitrarily setting all walk 'is.cycle' to FALSE.")
        }
...
gWalks = R6Class("gWalks",
                 public=list(
                     ...
                     initialize = function(grl=NULL, segs=NULL, paths=NULL,
                         ...
                         if (!is.null(segs)){
                             private$gwFromScratch(segs, paths, is.cycle, cn, str, metacols=metacols)
                         } 
                         else if (!is.null(grl)) {
