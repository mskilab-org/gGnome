...
The easiest way to visualize your own data is to use the `gen_gGnomejs_instance` function in the `gGnome` package.
...
We then generate the table required by `gen_gGnomejs_instance`:

```{r cache=FALSE,warning=FALSE}
jsdt = data.table(pair = c('hcc1954', 'h526'), complex = c('hcc1954.rds', 'h526.rds'), cov = c(hcc1954.cov.file, ''))
```
...
```{r cache=FALSE,warning=FALSE}
gen_gGnomejs_instance(data = jsdt,
...
Here is an outline of the steps done by `gen_gGnomejs_instance`:
...
To generate the genome graph JSON files and the coverage CSV files `gen_gGnomejs_instance` uses `gGnome` functions/methods. You can use these functions directly to add files to your gGnome.js folder.
...
We plan to soon add an option to `gen_gGnomejs_instance` to automatically output certain walks along with the genome graph.
