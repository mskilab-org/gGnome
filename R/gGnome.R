...
gNode = R6::R6Class("gNode",
                    ...
                    active = list(
                      ...
                      #' @name loose.left
                      #' @description
                      #' sets or accesses a logical vector specifying whether the given node has a loose.end on its left or right
                      #' 
...
gGraph = R6::R6Class("gGraph",
                     public = list(
                       ...
                       walks = function(greedy = FALSE, verbose = FALSE)
                       {
                         ...
                         paths = lapply(ap$paths, function(x) self$gr$snode.id[x])
                         cycles = lapply(ap$cycles, function(x) self$gr$snode.id[x])
                         

                         ## label each path / cycle
                         pstr = sapply(paths, function(x) gsub('\\-', 'x', paste(x, collapse = ' ')))
                         cstr = sapply(cycles, function(x) gsub('\\-', 'x', paste(rep(x), collapse = ' ')))


                         ## label reverse complement version
                         rpstr = sapply(paths, function(x) gsub('\\-', 'x', paste(rev(-x), collapse = ' ')))
                         rcstr = sapply(cycles, function(x) gsub('\\-', 'x', paste(rev(rep(-x,2)), collapse = ' ')))
                         ...
                         ced = cbind(seq_along(rcstr), vgrep(cstr, rcstr))
                       ...
                       {                         
                         ...
                         newedges[, ":="(n1 = dnodes$udnode.id[n1], n2 = dnodes$udnode.id[n2])]
                         ...
                         if (length(metacolsn)>0)
                         {
                           final.nodes = defactor(final.nodes)
                         ...
                         final.edges = defactor(final.edges)
                       ...
                       simplify = function(by = private$pmeta$by, na.rm = TRUE, avg = TRUE, sep = ',', FUN = default.agg.fun.generator(na.rm = na.rm, sep = sep, avg = avg), ignore.loose = FALSE)
                       {
                         ...
                         {
                           nodes.out = cbind(nodes.out, defactor(nodes.dt2)[, lapply(.SD, FUN), .SDcols = metadata.cols, by = eval(paste(by))][, metadata.cols, with = FALSE])
                         }
                       ...
                       gtrack = function(y.field = NULL, ...)
                       {
                         ...
                         if (is.null(ss$loose))
                         {
                           lleft = self$nodes$lleft[, 'snode.id']
                           lright = self$nodes$lright[, 'snode.id']
                         ...
                         gt.args = private$pmeta[intersect(c('colormaps',
                                                             'gr.colorfield',
                                                             'y.field',
                                                             'col',
                                                             'border',
...
gWalk = R6::R6Class("gWalk", ## GWALKS
                    public = list(
                      ...
                      set = function(...)
                       {
                         ...
                         NONO.FIELDS = c('walk.id', 'circular')
                         args = list(...)
                      ...
                      {
                        message("Coming soon! when implemented will allow you to simplify a set of gWalks and the gGraph that they are attached to")
                      },
                      ...
                      gtrack = function(name = NULL, stack.gap = 1e5, ...)
                      {
                        ...
                        gt.args = private$pgraph$meta[intersect(c('colormaps',
                                                            'gr.colorfield',
                                                            'col',
                                                            'border',
                                                            'name',
                    ...
                    private = list(
                                              ...
                                              circular = NULL)  ## logical vector specifying which contigs are circular, i.e. have an implied edge from the final node to the first                                            
                      {
                          ...
                          if (!is.null(meta))
                          {
                            ...
                            GW.NONO = c('walk.id', 'circular', 'snode.id', 'sedge.id')
                            good.cols = setdiff(names(meta), GW.NONO)

                            if (length(good.cols)>0)
                              {                            
                                private$pmeta = cbind(data.table(walk.id = 1:length(snode.id)), meta[, good.cols, with = FALSE])
                              }
                          }
