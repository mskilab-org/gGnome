...
```balance``` is a general function for inferring genome graphs representing genomic structural variation by integrating read depth and junction call data. ```balance``` is used for cancer genome graph inference in our package [JaBbA](https://github.com/mskilab/JaBbA). 

```balance``` takes as input a gGraph with segment copy number estimates stored in the node metadata and solves a Mixed Integer Program (MIP) to infer integer node and edge copy numbers. It is possibe to perform this optimization as either a mixed-integer quadratic program (MIQP) or a mixed-integer linear program (MILP). JaBbA used a quadratic program at the time of publication, but has since been modified to used linear programming due to improved speed.To run ```balance```, users will need to install [CPLEX](https://www.ibm.com/analytics/cplex-optimizer).
...
plot(c(sg, sg.cov.gt), rg)
```

Note that in plotted image, the "y" location of the subgraph graph nodes is not reflective of copy number.

<img src="./ggnome/hcc1954.unbalanced.png" alt="unbalanced hcc1954" width="500"/>
...
GRanges object with 4 ranges and 9 metadata columns:
  ...
  1         4         1         1         1   4.67980 37631.4942
  2         4         2         2         2   3.31639  1567.1930
  3         4         3         3         3   1.92122    46.6659
  4         4         4         4         4   3.41781   533.3072
  -------
...
<img src="./ggnome/hcc1954.balanced.png" alt="unbalanced hcc1954" width="500"/>
